...
export default function Contact() {
  ...
  const getEmbedUrl = (url: string) => {
    ...
    try {
      ...
      // If can't parse, return a default location or null
      return null
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            {/* Social Media */}
            {(config?.socialMedia?.facebook || config?.socialMedia?.twitter || config?.socialMedia?.linkedin || config?.socialMedia?.instagram || config?.socialMedia?.youtube) && (
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex gap-3 flex-wrap">
                  {config?.socialMedia?.facebook && (
                    <a 
                      ...
                      className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 transition-all duration-300 transform hover:scale-110"
                      title="Facebook"
                    >
                  ...
                  {config?.socialMedia?.twitter && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="w-10 h-10 bg-sky-500 text-white rounded-full flex items-center justify-center hover:bg-sky-600 transition-all duration-300 transform hover:scale-110"
                      title="Twitter / X"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {config?.socialMedia?.linkedin && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="w-10 h-10 bg-blue-700 text-white rounded-full flex items-center justify-center hover:bg-blue-800 transition-all duration-300 transform hover:scale-110"
                      title="LinkedIn"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {config?.socialMedia?.instagram && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="w-10 h-10 bg-gradient-to-br from-purple-600 via-pink-600 to-orange-500 text-white rounded-full flex items-center justify-center hover:opacity-90 transition-all duration-300 transform hover:scale-110"
                      title="Instagram"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  ...
                  {config?.socialMedia?.youtube && (
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer" 
                      className="w-10 h-10 bg-red-600 text-white rounded-full flex items-center justify-center hover:bg-red-700 transition-all duration-300 transform hover:scale-110"
                      title="YouTube"
                    >
        ...
        {config?.mapUrl && (
          <div className="bg-white rounded-lg shadow-md p-4">
            ...
            {getEmbedUrl(config.mapUrl) ? (
              <iframe
                src={getEmbedUrl(config.mapUrl) || ''}
                width="100%"
                height="450"
                style={{ border: 0 }}
                allowFullScreen
              ...
              <div className="bg-gray-100 rounded-lg p-8 text-center">
                ...
                <p className="text-gray-600 mb-4">Map URL needs to be configured properly</p>
                <a
                  href={config.mapUrl}
                  target="_blank"
                  rel="noopener noreferrer"
